...
def swag_parameters(module, params, no_cov_mat=False):
...
class SWAG(torch.nn.Module):
    def __init__(self, base, no_cov_mat=False, max_num_models=0, var_clamp=1e-30): #*args, **kwargs: dropped
    ...
    # cov: default is modified from False to True
    # scale : default is modified from 1.0 to 0.5
    def sample(self, scale=0.5, cov=True, seed=None, block=False, fullrank=True, device=str):
        if seed is not None:
            torch.manual_seed(seed)
        if not block:
            self.sample_fullrank(scale, cov, fullrank, device)
        else:
    ...
    def sample_fullrank(self, scale, cov, fullrank, device=str):
    ...
    def chunk_sample(self, scale=1.0, cov=True, chunk_size=None, num_chunks=None):
        ...
        sample = mean + scale_sqrt * rand_sample
        sample = sample.unsqueeze(0)
        remainder = chunk_size * num_chunks - sample.size(1)
        zeros = torch.zeros(1, remainder)

        sample = torch.cat((sample, zeros), dim=1)
        mean_ = torch.cat((mean.unsqueeze(0), zeros), dim=1)
        var_ = torch.cat((var.unsqueeze(0), zeros), dim=1)

        sample = sample.view(num_chunks, -1)
        mean_ = mean_.view(num_chunks, -1)
        var_ = var_.view(num_chunks, -1)

        means = torch.mean(mean_, dim=1).view(num_chunks, 1)
        stds = torch.sqrt(torch.mean(var_, dim=1).view(num_chunks, 1))

        sample = (sample-means) / stds

        return sample, remainder, means, stds

    def get_entropy(self, cov=False, chunk_size=None, num_chunks=None):
        ...
        # draw diagonal variance sample
        var = torch.clamp(sq_mean - mean ** 2, self.var_clamp)
        entropy = 0.5 * (1 + torch.log(2*np.pi*var))
        entropy = entropy.unsqueeze(0)
        var = var.unsqueeze(0)
        remainder = chunk_size * num_chunks - entropy.size(1)

        zeros = torch.zeros(1, remainder)

        entropy = torch.cat((entropy, zeros), dim=1)
        entropy = entropy.view(num_chunks, -1)
        entropy_last_mean = torch.mean(entropy[-1][:chunk_size-remainder])
        entropy_mean = torch.mean(entropy, dim=1)
        entropy_mean[-1] = entropy_last_mean

        var = torch.cat((var, zeros), dim=1)
        var = var.view(num_chunks, -1)
        var_last_mean = torch.mean(var[-1][:chunk_size - remainder])
        var_mean = torch.mean(var, dim=1)
        var_mean[-1] = var_last_mean

        return entropy_mean, var_mean
